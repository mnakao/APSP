...
#ifdef _MPI
void init_owner_table(const int nodes)
{
  owner_table = malloc(sizeof(int) * nodes);
  int chunk = (nodes%in_bfs_procs==0)? nodes/in_bfs_procs : nodes/in_bfs_procs + 1;

  int owner_id = 0;
  int i = 0, j = 1;
  while(1){
    owner_table[i++] = owner_id;
    if(i>=chunk*j){
      owner_id++;
      j++;
    }
...
void lower_bound_of_diam_aspl_grid(int *low_diam, double *low_ASPL, const int m, const int n,
...
void create_adjacency(const int nodes, const int lines, const int degree,
		      int edge[lines][2], int adjacency[nodes][degree], int num_degrees[nodes])
{
  for(int i=0;i<lines;i++){
    int n1 = edge[i][0];
    int n2 = edge[i][1];
