		       ...
		       int groups, int *diameter, double *ASPL, double *sum)
{
  ...
  int *lev      = malloc(sizeof(int) * nodes);
  ...
  for(int r=0;r<nodes;r++){
    ...
    clear_buffer(nodes, lev,     NOT_VISITED);
    lev[r] = 0;
    bitmap[r] = 1;
...
#pragma omp for
	for(int c=0;c<nodes;c++){
	  if(lev[c] != level) continue;
	  for(int i=0;i<degree;i++){
	    int n = adjacency[c][i];
	    if(bitmap[n] == 0){
	      if(__sync_bool_compare_and_swap(&bitmap[n], 0, 1)){
		lev[n] = level + 1;
		fin[tid] = false;
		parents[n] = parents[c]+1;
	      }
	    }
	  }
	}
